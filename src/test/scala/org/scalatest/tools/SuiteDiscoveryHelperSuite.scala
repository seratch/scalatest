...
class SuiteDiscoveryHelperFriend(sdt: SuiteDiscoveryHelper.type) {
  ...
  Set[String] =
  {
    val m = Class.forName("org.scalatest.tools.SuiteDiscoveryHelper$").getDeclaredMethod("org$scalatest$tools$SuiteDiscoveryHelper$$processFileNames",
      Array(classOf[Iterator[String]], classOf[Char], classOf[ClassLoader], classOf[Option[Pattern]], classOf[ServiceProviderManager]): _*)
    m.setAccessible(true)
    m.invoke(sdt, Array[Object](fileNames, new java.lang.Character(fileSeparator), loader, suffixes, new ServiceProviderManager(loader)): _*).asInstanceOf[Set[String]]
  }
