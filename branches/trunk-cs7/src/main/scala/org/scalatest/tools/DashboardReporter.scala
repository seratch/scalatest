  ...
  class TestRecord(startEvent: TestStarting) {
    ...
    object Duration {
      def unapply(event: Event): Option[Long] =
        event match {
          case TestSucceeded(_, _, _, _, _, _, _, _, _, duration, _, _, _, _, _, _)
            => duration
          case TestFailed(_, _, _, _, _, _, _, _, _, _, _, duration, _, _, _, _, _, _) 
            => duration
          case TestPending(_, _, _, _, _, _, _, _, _, duration, _, _, _, _, _) 
            => duration
          case TestCanceled(_, _, _, _, _, _, _, _, _, _, _, duration, _, _, _, _, _) 
            => duration
          case _ => None
        }
    }
