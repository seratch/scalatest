...
class RunNotifierSuite extends FunSuite {
  ...
  test("report(TestFailed) generates a fireTestFailure invocation") {
    ...
    reporter(TestFailed(ordinal, "No msg", "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, "theTestName", "theTestName", None, None, 0, Some(exception)))
    assert(runNotifier.passed.get.getDescription.getDisplayName === "theTestName(fully.qualified.SuiteClassName)")
    reporter(TestFailed(ordinal, "No msg", "SuiteClassName", "suite ID", None, None, "theTestName", "theTestName", None, None, 0, Some(exception)))
  ...
  test("report(TestSucceeded) generates a fireTestFinished invocation") {
    ...
    val reporter = new RunNotifierReporter(runNotifier)
    reporter(TestSucceeded(ordinal, "SuiteClassName", "suite ID", Some("fully.qualified.SuiteClassName"), None, "theTestName", "theTestName", None, None, 0))
    assert(runNotifier.passed.get.getDisplayName === "theTestName(fully.qualified.SuiteClassName)")
    reporter(TestSucceeded(ordinal, "SuiteClassName", "suite ID", None, None, "theTestName", "theTestName", None, None, 0))
