...
write a sbt plugin to deploy the task.
    ...
    def run(testClassName: String, fingerprint: Fingerprint, eventHandler: EventHandler, args: Array[String]) {
      ...
      if (isAccessibleSuite(testClass) || isRunnable(testClass)) {
        ...
        report(SuiteStarting(tracker.nextOrdinal(), suite.suiteName, suite.suiteId, Some(testClass.getName), suite.decodedSuiteName, formatter, Some(TopOfClass(testClass.getName))))
        ...
        try {
          ...
          report(SuiteCompleted(tracker.nextOrdinal(), suite.suiteName, suite.suiteId, Some(testClass.getName), suite.decodedSuiteName, Some(duration), formatter, Some(TopOfClass(testClass.getName))))
        }
        catch {       
          case e: Exception => {
            ...
            val duration = System.currentTimeMillis - suiteStartTime
            report(SuiteAborted(tracker.nextOrdinal(), rawString, suite.suiteName, suite.suiteId, Some(testClass.getName), suite.decodedSuiteName, Some(e), Some(duration), formatter, Some(SeeStackDepthException)))
          }
        }
      }
