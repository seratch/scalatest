...
class RunnerSuite() extends Suite with PrivateMethodTester {
  ...
  def testParseReporterArgsIntoSpecs() {
    ...
    expectResult(new ReporterConfigurations(None, Nil, Nil, Nil, Nil, None, None, List(new HtmlReporterConfiguration(Set(), "htmldir", None), new HtmlReporterConfiguration(Set(), "html", Some(new File("MyStyle.css").toURI.toURL))), Nil, Nil)) {
